...
public class SubscriptionManager {
	...
	public void subscribe(String channelName, AbstractSubscriber subscriber) {
		...
		// if not blocked, subscribe subscriber to channel
		if (ChannelAccessControl.getInstance().checkIfBlocked(subscriber, channelName) == false) {
			AbstractChannel channel = cpManager.findChannel(channelName);
			channel.subscribe(subscriber);
			System.out.println("Subscriber " + subscriber.hashCode() + " subscribes to " + channelName );
		}	
	}
